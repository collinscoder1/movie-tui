...
import type { WcoSourceClient } from '../../../src/source/providers/wco/source.js';
import type { SourceTvSeasonInfo } from '../../../src/source/types.js';
import type { DownloadEntry } from '../../../src/extractor.js';
...
test('WCO provider fetchDownloads returns grouped MP4 entries', async () => {
  const provider = createWcoSource(client);
  const seasonInfo = (await provider.describeFromTmdb('tv', DETAIL_URL)) as SourceTvSeasonInfo;
  ...
  const mp4Entries: DownloadEntry[] = result.downloads.MP4;
