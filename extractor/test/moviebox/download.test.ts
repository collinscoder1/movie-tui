...
async function fetchFirstChunk(url: string) {
  ...
  if (view.length >= 8) {
    ...
    if (isMp4) console.log('    container detected: MP4');
    if (isMkv) console.log('    container detected: MKV/WebM');
  }
}
...
const episodeNumber = isTvLike ? seasonCandidate.episodes[0].episode : 0;
        ...
        assert.ok(
          downloadResult.hasResource && downloadResult.downloads.length > 0,
          'moviebox download endpoint should return at least one stream'
        );
