...
export async function checkExistingDownloads(
  descriptors: EpisodeDescriptor[],
  baseFolder?: string
): Promise<{ downloaded: EpisodeDescriptor[]; missing: EpisodeDescriptor[] }> {
  const downloaded: EpisodeDescriptor[] = [];
  const missing: EpisodeDescriptor[] = [];
