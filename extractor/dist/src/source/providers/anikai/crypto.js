import { DECODE_CHAIN, RC4_KEY_SOURCES, STAGE_SPECS, XOR_KEYS } from './crypto-data.js';
const compiledStages = new Map();
const compiledRc4Keys = new Map();
...
function stringToLatin1Bytes(input) {
...
function applyDecodeStage(input, stageName) {
...
function applyEncodeStage(output, stageName) {
    const stage = getCompiledStage(stageName);
    const skipCount = stage.prefix / 2;
    const inputLength = output.length < skipCount ? output.length * 2 : output.length + skipCount;
    ...
    for (let outputIndex = 0; outputIndex < output.length; outputIndex += 1) {
        const inputIndex = outputIndex < skipCount
            ? outputIndex * 2 + 1
