...
function buildCookieSensitiveKey(seedBase64, options = {}) {
    ...
    const querySelectorNative = options.querySelectorNative === undefined ? true : options.querySelectorNative;
...
function decodeMediaStagesNative(token, options = {}) {
    ...
    const environment = {
        ...
        querySelectorNative: options.querySelectorNative === undefined
            ? true
...
async function requestText(url, headers, fetchImpl, signal) {
...
export async function resolveMegaupMediaPayload(embedUrl, options = {}) {
    ...
    const responseText = await requestText(mediaUrl, {
        ...MEDIA_HEADERS,
        cookie: cookieHeader,
        origin: url.origin,
        referer: embedUrl
...
export async function resolveMegaupEmbedDownload(embedUrl, options = {}) {
    const fetchImpl = options.fetchImpl ?? fetch;
...
export async function resolveMegaupFinalDownloads(embedUrl, options = {}) {
    const fetchImpl = options.fetchImpl ?? fetch;
    ...
    const pageHtml = await requestText(downloadPageUrl, {
        ...PAGE_HEADERS,
...
export { USER_AGENT_CHROME, computeMegaupCookieName, createMegaupCookie, decodeMediaResultNative, decodeMediaStagesNative, parseCookieHeader, convertCheckToDownload, decodeMegaupDataUrl, extractMegaupDownloadEntries };
