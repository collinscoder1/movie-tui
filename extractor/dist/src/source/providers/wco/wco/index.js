...
function extractEpisodes(document, baseUrl) {
    const episodes = [];
    const primaryNodes = document('div.cat-eps');
    const pushFromAnchor = (anchor, fallbackTitle) => {
...
function isWcoRelatedUrl(url) {
    return url.includes('wcoflix.tv') ||
        url.includes('wcostream.com') ||
        url.includes('/getvid?evid=');
}
async function performFetch(url, init, customFetchImpl) {
...
export async function searchSeries(keyword, options = {}) {
...
export async function getSeriesDetail(seriesUrl, options = {}) {
    const fetchImpl = options.fetchImpl;
...
function createLinkInfoBase(embedOrigin) {
    if (embedOrigin.includes('/inc/embed')) {
        return embedOrigin.split('/inc/embed')[0];
    }
    return embedOrigin;
}
async function resolveCdnUrl(server, evid, embedUrl) {
...
export async function getVideoInfo(episodeUrl, options = {}) {
    const fetchImpl = options.fetchImpl;
