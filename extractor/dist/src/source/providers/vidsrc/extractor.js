...
export function parseVidsrcUrl(value) {
...
function buildFriendlyName({ type, title, year, season, episode }) {
...
async function fetchMovieMetadata(tmdbId, fetchImpl = globalThis.fetch) {
...
async function fetchTvMetadata(tmdbId, season, episode, fetchImpl = globalThis.fetch) {
...
async function invokeDownloadProxy(payload, friendlyName) {
    const client = payload.supabaseClient ?? defaultSupabaseClient;
    const { data: tokenData, error: tokenError } = await client.functions.invoke('get-token');
    if (tokenError) {
        throw tokenError;
    }
    const requestToken = tokenData?.t ?? '';
    const { data: proxyData, error } = await client.functions.invoke('download-proxy', {
    ...
    if (extractData?.streams) {
        for (const stream of extractData.streams) {
            ...
            downloads.push({
                ...
                url: buildProxyUrl(stream.url, friendlyName)
            });
        }
    }
    if (extractData?.downloads) {
        for (const entry of extractData.downloads) {
            ...
            downloads.push({
                ...
                format: 'MP4',
                size: formatSize(sizeRaw),
                url: buildProxyUrl(entry.url, friendlyName)
            });
        }
    }
    ...
    const grouped = downloads.reduce((acc, entry) => {
    ...
    if (extractData?.captions) {
        ...
        for (const caption of extractData.captions) {
            ...
            subtitles.push({
                ...
                size: formatSize(sizeRaw),
                url: `https://sub.k5s7sjozpn.workers.dev/?url=${encodeURIComponent(caption.url)}&title=${encodeURIComponent(friendlySubtitleName)}`
            });
        }
    }
