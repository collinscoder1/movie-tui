...
test('moviebox search uses live data', async (t) => {
    try {
        ...
        assert.ok(result.items.some((item) => item.detailPath === DETAIL_PATH), `search results should include ${DETAIL_PATH}`);
    }
    catch (err) {
...
test('detail payload yields seasons and episodes from live data', async (t) => {
    try {
        ...
        assert.ok(detail.seasons.some((season) => season.episodes.length > 0), 'at least one season should expose episodes');
    }
    catch (err) {
        handleNetworkError(t, err);
        return;
